...
/*void Parametre::setDate(const QDate dd){
    QStringList Liste = Test->getListDate();
    int i=0;
    bool CheckDate=0;
    while(Liste[i]!=NULL)
    {
        if(dd.toString()==Liste[i]){}
            CheckDate=1;
    }
    if(CheckDate == 0)throw ParametreException("Date non disponible");
}*/

CoursOHLC::CoursOHLC(double o, double h, double l, double c, double v, const QDate& d):date(d) {
    if (o < 0 || h < 0 || l < 0 || c<0 || v<0 || l>h) throw TradingException("cours OHLC incorrect");
...
const Devise& DevisesManager::creationDevise(const QString& c, const QString& m, const QString& z) {
    ...
    // creation et ajout de la nouvelle devise...
...
const PaireDevises& DevisesManager::getPaireDevises(const QString & c1, const QString & c2) const {
    ...
    for (unsigned int i = 0; i < nbPaires; i++)
        if (paires[i]->getBase().getCode() == c1 &&
            ...
            return *paires[i];
    // si la paire de devises n'est pas trouvee, il faut la creer
