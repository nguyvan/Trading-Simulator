...
class ParametreException {
public:
    ParametreException(const QString& message) :info(message) {}
    QString getInfo() const { return info; }
private:
    QString info;
};
...
/*class Parametre {
    const Reader* Test;
    QDate datedebut = Test->getDate();
    double deviseBase = 0;
    double deviseCotation = 1000000;
public:
    Parametre(){}
    Parametre(const Reader& P , double dB, double dC);
    double getDeviseBase() const {return deviseBase ;}
    double getDeviseCotation() const {return deviseCotation ;}
    void setDeviseBase(double dB) {deviseBase = dB;}
    void setDeviseCotation(double dC) {deviseBase = dC;}
    QDate getDate() const { return Test->getDate(); }
